...
//want to add dnssec record types
...
//https://tools.ietf.org/html/rfc4034#section-6.4
...
/**
 * [dnssec description]
 * @type {Object} dnssec    [description]
 * @type {Object} dnssec.DNSKEY [description]
 * @type {Object} dnssec.RRSIG  [description]
 * @type {Object} dnssec.NSEC   [description]
 * @type {Object} dnssec.NSEC3  [description]
 * @type {Object} dnssec.NSEC3PARAM [description]
 * @docs https://tools.ietf.org/html/rfc4034
 */
Packet.Resource.DNSKEY = {
    decode: function(reader, length) {
        this.flags = reader.read(16); // 2 octets
        this.protocol = reader.read(8); // 1 octet
        this.algorithm = reader.read(8); // 1 octet
        this.publicKey = reader.read(length - 4); // variable length
        return this;
    },
...
Packet.Resource.RRSIG = {
    decode: function(reader, length) {
        this.typeCovered = reader.read(16); // 2 octets
        this.algorithm = reader.read(8); // 1 octet
        this.labels = reader.read(8); // 1 octet
        this.originalTTL = reader.read(32); // 4 octets
        this.signatureExpiration = reader.read(32); // 4 octets
        this.signatureInception = reader.read(32); // 4 octets
        this.keyTag = reader.read(16); // 2 octets
        this.signersName = Packet.Name.decode(reader); // variable length
        this.signature = reader.read(length - 26 - this.signersName.length); // variable length
        return this;
    },
    encode: function(record, writer) {
        writer = writer || new Packet.Writer();
        writer.write(record.typeCovered, 16);
        writer.write(record.algorithm, 8);
        writer.write(record.labels, 8); // 1 octet
        writer.write(record.originalTTL, 32); // 4 octets
        writer.write(record.signatureExpiration, 32); // 4 octets
        writer.write(record.signatureInception, 32); // 4 octets
        writer.write(record.keyTag, 16); // 2 octets
        Packet.Name.encode(record.signersName, writer); // variable length
        writer.write(record.signature); // variable length
        return writer.toBuffer();
    },
};
Packet.Resource.NSEC = {
    decode: function(reader, length) {
        this.nextDomainName = Packet.Name.decode(reader); // variable length
        this.typeBitMaps = reader.read(length - this.nextDomainName.length); // variable length
        return this;
    },
    encode: function(record, writer) {
        writer = writer || new Packet.Writer();
        Packet.Name.encode(record.nextDomainName, writer); // variable length
        writer.write(record.typeBitMaps); // variable length
        return writer.toBuffer();
    },
};
Packet.Resource.NSEC3 = {
    decode: function(reader, length) {
        this.hashAlgorithm = reader.read(8); // 1 octet
        this.flags = reader.read(8); // 1 octet
        this.iterations = reader.read(16); // 2 octets
        this.saltLength = reader.read(8); // 1 octet
        this.salt = reader.read(this.saltLength); // variable length
        this.nextHashedOwnerNameLength = reader.read(8); // 1 octet
        this.nextHashedOwnerName = reader.read(this.nextHashedOwnerNameLength); // variable length
        this.typeBitMaps = reader.read(length - 5 - this.saltLength - this.nextHashedOwnerNameLength); // variable length
        return this;
    },
    encode: function(record, writer) {
        writer = writer || new Packet.Writer();
        writer.write(record.hashAlgorithm, 8); // 1 octet
        writer.write(record.flags, 8); // 1 octet
        writer.write(record.iterations, 16); // 2 octets
        writer.write(record.saltLength, 8); // 1 octet
        writer.write(record.salt); // variable length
        writer.write(record.nextHashedOwnerNameLength, 8); // 1 octet
        writer.write(record.nextHashedOwnerName); // variable length
        writer.write(record.typeBitMaps); // variable length
        return writer.toBuffer();
    },
};
Packet.Resource.NSEC3PARAM = {
    decode: function(reader, length) {
        this.hashAlgorithm = reader.read(8); // 1 octet
        this.flags = reader.read(8); // 1 octet
        this.iterations = reader.read(16); // 2 octets
